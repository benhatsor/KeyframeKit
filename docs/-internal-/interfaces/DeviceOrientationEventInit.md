[**KeyframeKit.js**](../../README.md)

***

[KeyframeKit.js](../../globals.md) / [\<internal\>](../README.md) / DeviceOrientationEventInit

# Interface: DeviceOrientationEventInit

Defined in: node\_modules/typescript/lib/lib.dom.d.ts:556

## Extends

- [`EventInit`](EventInit.md)

## Properties

### absolute?

> `optional` **absolute**: `boolean`

Defined in: node\_modules/typescript/lib/lib.dom.d.ts:557

***

### alpha?

> `optional` **alpha**: `number` \| `null`

Defined in: node\_modules/typescript/lib/lib.dom.d.ts:558

***

### beta?

> `optional` **beta**: `number` \| `null`

Defined in: node\_modules/typescript/lib/lib.dom.d.ts:559

***

### bubbles?

> `optional` **bubbles**: `boolean`

Defined in: node\_modules/typescript/lib/lib.dom.d.ts:662

#### Inherited from

[`EventInit`](EventInit.md).[`bubbles`](EventInit.md#bubbles)

***

### cancelable?

> `optional` **cancelable**: `boolean`

Defined in: node\_modules/typescript/lib/lib.dom.d.ts:663

#### Inherited from

[`EventInit`](EventInit.md).[`cancelable`](EventInit.md#cancelable)

***

### composed?

> `optional` **composed**: `boolean`

Defined in: node\_modules/typescript/lib/lib.dom.d.ts:664

#### Inherited from

[`EventInit`](EventInit.md).[`composed`](EventInit.md#composed)

***

### gamma?

> `optional` **gamma**: `number` \| `null`

Defined in: node\_modules/typescript/lib/lib.dom.d.ts:560
