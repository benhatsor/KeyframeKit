...
var KeyframesFactory = class {
	...
	*/
	async importStyleSheet(url) {
		...
		if (!resp.ok) throw new this.Error.StyleSheetImportError();
		const respText = await resp.text();
		const baseURL = url.split("/").slice(0, -1).join("/");
		const styleSheet = new CSSStyleSheet({ baseURL });
	...
	*/
	getStyleSheetKeyframes({ of: ruleName, in: source }) {
		if (typeof ruleName !== "string") throw new this.Error.KeyframesRuleNameTypeError();
		if (source instanceof StyleSheetList) return this.#getStyleSheetKeyframesInStyleSheetList({
			of: ruleName,
			styleSheetList: source
		});
		else if (source instanceof CSSStyleSheet) return this.#getStyleSheetKeyframesInStyleSheet({
			of: ruleName,
			styleSheet: source
		});
		else throw new this.Error.SourceTypeError();
	}
	...
	*/
	getAllStyleSheetKeyframesRules({ in: source }) {
		if (source instanceof StyleSheetList) return this.#getAllStyleSheetKeyframesRulesInStyleSheetList({ styleSheetList: source });
		else if (source instanceof CSSStyleSheet) return this.#getAllStyleSheetKeyframesRulesInStyleSheet({ styleSheet: source });
		else throw new this.Error.SourceTypeError();
	}
	#getAllStyleSheetKeyframesRulesInStyleSheetList({ styleSheetList }) {
		let keyframesRules = {};
		for (const styleSheet of styleSheetList) {
			const styleSheetKeyframesRules = this.#getAllStyleSheetKeyframesRulesInStyleSheet({ styleSheet });
			keyframesRules = {
				...keyframesRules,
	...
	#getAllStyleSheetKeyframesRulesInStyleSheet({ styleSheet }) {
		...
		for (const rule of styleSheet.cssRules) {
			if (!(rule instanceof CSSKeyframesRule)) continue;
			const keyframes = this.parseKeyframesRule({ rule });
	...
	*/
	parseKeyframesRule({ rule: keyframes }) {
		...
		for (const keyframe of keyframes) {
			...
			const offset = Number(percentString) / 100;
		...
		return new ParsedKeyframes(parsedKeyframes);
	}
	...
	#cssPropertyToIDLAttribute(property, lowercaseFirst = false) {
		...
		if (lowercaseFirst) property = property.slice(1);
		for (const c of property) if (c === CHARS.HYPHEN_MINUS) uppercaseNext = true;
	...
	#isCustomPropertyName(property) {
		return property.startsWith(CHARS.DOUBLE_HYPHEN_MINUS) && property !== CHARS.DOUBLE_HYPHEN_MINUS;
	}
...
var src_default = new KeyframesFactory();
/** https://drafts.csswg.org/web-animations-1/#the-keyframeeffect-interface */
var KeyframeEffectParameters = class {
	keyframes;
	options;
	/**
	* @param obj
	...
	*/
	toAnimation({ target, options: additionalOptions = {}, timeline = document.timeline }) {
		...
		return new Animation(keyframeEffect, timeline);
	}
...
var ParsedKeyframes = class {
	...
	*/
	toKeyframeEffect(options) {
		let keyframeEffect;
		if (options !== null) keyframeEffect = new KeyframeEffectParameters({
			keyframes: this.keyframes,
			options
		});
		else keyframeEffect = new KeyframeEffectParameters({ keyframes: this.keyframes });
...
async function waitForDocumentLoad({ document }) {
	...
	const listener = ["readystatechange", onReadyStateChange];
	document.addEventListener(...listener);
	await promise;
	document.removeEventListener(...listener);
}
//#endregion
export { KeyframeEffectParameters, ParsedKeyframes, src_default as default };
